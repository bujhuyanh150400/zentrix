...
const TransactionTabs: FC<Props> = (props) => {
  ...
  const hookCalculate = useCalculateTransactionPrices(
    transactionsData,
    filter.status === _TransactionStatus.OPEN ||
      filter.status === _TransactionStatus.WAITING
  );
    ...
    <>
      ...
      >
        {query.isLoading || query.isRefetching ? (
          <SkeletonCardSymbol numberCard={2} />
        ) : (
          <>
            {hookCalculate.data && hookCalculate.data.length > 0 ? (
                  ...
                  <>
                    ...
                    >
                      <Paragraph
                        ...
                        theme="alt2"
                      >
                        Hiển thị các giao dịch đóng trong vòng 30 ngày trở lại
                        gần đây
                      </Paragraph>
                    </YStack>
                  </>
              ...
              >
                ...
                <Paragraph textAlign="center" theme="alt2">
                  Tận dụng cơ hội giao dịch trên các thị trường tài chính lớn
                  trên thế giới
                </Paragraph>
              </YStack>
            )}
      ...
      <YStack gap={"$2"}>
        ...
        <XStack gap={"$1"} alignItems={"center"}>
          <Paragraph
            ...
            fontWeight={500}
            color={
              item.type === _TradeType.BUY
                ? DefaultColor.blue[500]
                : DefaultColor.red[500]
            }
          >
            {item.type === _TradeType.BUY ? "Mua" : "Bán"}{" "}
            {item.volume.toFixed(2)}{" "}
            {item.type_trading === _TypeTrading.USD ? "USD" : "Lô"}
          </Paragraph>
          {item.entry_price && (
            <Paragraph
              fontSize={14}
...
const TransactionInfoSheet: FC<{
  ...
  const onCloseTransaction = useCallback(
    (data: CalculateTransactionPrices) => {
      Alert.alert(
        "Đóng giao dịch",
        "Bạn có chắc chắn muốn đóng giao dịch này?",
        [
  ...
  const onOpenNowTransaction = useCallback(
    (data: CalculateTransactionPrices) => {
      Alert.alert(
        "Thực hiện mở giao dịch",
        "Giao dịch đang chờ để chốt giá mong muốn, bạn muốn mở luôn giao dịch ?",
        [
          { text: "Hủy", style: "cancel" },
          {
            text: "Mở giao dịch",
  ...
  const onCancelTransaction = useCallback(
    (data: CalculateTransactionPrices) => {
      Alert.alert(
        "Xóa giao dịch",
        "Giao dịch đang chờ để chốt giá mong muốn, bạn không muốn thực hiện giao dịch này nữa ?",
        [
          { text: "Hủy", style: "cancel" },
          {
            text: "Xóa giao dịch",
          ...
          <>
            <YStack gap={"$2"}>
              ...
              <XStack alignItems={"center"} justifyContent={"space-between"}>
                ...
                <Paragraph fontWeight={500} color={DefaultColor.slate[700]}>
                  {data.open_at
                    ? new Date(data.open_at).toLocaleString()
                    : "__"}
                </Paragraph>
              </XStack>
              {/*Thời gian đóng*/}
              <XStack alignItems={"center"} justifyContent={"space-between"}>
                ...
                <Paragraph fontWeight={500} color={DefaultColor.slate[700]}>
                  {data.close_at
                    ? new Date(data.close_at).toLocaleString()
                    : "__"}
                </Paragraph>
              </XStack>
              {/*Giá chốt */}
              ...
              <XStack alignItems={"center"} justifyContent={"space-between"}>
                ...
                <Paragraph fontWeight={500} color={DefaultColor.slate[700]}>
                  {data.rate_to_usd
                    ? `${data.rate_to_usd.toFixed(2)} USD`
                    : "__"}
                </Paragraph>
              </XStack>
            </YStack>
            ...
            {data.status === _TransactionStatus.WAITING && (
              <XStack alignItems={"center"} gap={"$2"}>
                <Button
                  flex={1}
                  disabled={
                    mutationOpenNowTrans.isPending ||
                    mutationCancelTrans.isPending
                  }
                  marginTop={"$2"}
                  onPress={() => {
