...
const TransactionSheet: FC<TransactionSheetProps> = (props) => {
  ...
  const [tab, setTab] = useState<_TransactionTriggerType>(
    _TransactionTriggerType.TYPE_TRIGGER_AUTO_TRIGGER
  );
  ...
  const hookHistory = useTransactionHistory({
    ...
    status: [
      ...
      _TransactionTriggerType.TYPE_TRIGGER_LOW_BUY,
    ].includes(form.type_trigger)
    ...
    if (
      ...
      if (form.percent_stop_loss) {
        ...
        if (stopLoss < 0 || stopLoss > 100) {
          setError({
            percent_stop_loss: "Cắt lỗ phải lớn hơn 0 hoặc nhỏ hơn 100%",
            isError: true,
          });
        } else {
      ...
      if (form.percent_take_profit) {
        ...
        if (stopLoss < 0 || stopLoss > 100) {
          setError({
            percent_take_profit: "Chốt lời phải lớn hơn 0 hoặc nhỏ hơn 100%",
            isError: true,
          });
        } else {
    ...
    if (form.trigger_price) {
      if (form.type_trigger === _TransactionTriggerType.TYPE_TRIGGER_LOW_BUY) {
        if (parseToNumber(form.trigger_price) >= props.price) {
          setError({
            trigger_price: `Giá mua thấp phải nhỏ hơn ${props.price}`,
            isError: true,
          });
        } else {
        ...
        form.type_trigger === _TransactionTriggerType.TYPE_TRIGGER_HIGH_BUY
      ) {
        if (parseToNumber(form.trigger_price) <= props.price) {
          setError({
            trigger_price: `Giá mua cao phải lớn hơn ${props.price}`,
            isError: true,
          });
        } else {
          setError({ trigger_price: ``, isError: false });
  ...
  useEffect(() => {
    if (props.open) {
      if (props.symbol && props.account) {
        // set account, symbol, and the current trade type when opening so the form
        // doesn't keep a stale `type` value from the previous open
        setForm({
  ...
  }, [props.symbol, props.account, props.open, props.tradeType, props.price]);
  useEffect(() => {
    if (openMore) {
      setForm({
  ...
  return (
    <>
                  ...
                  <XStack gap={"$2"}>
                    <TouchableOpacity
                      style={[
                        ...
                        {
                          backgroundColor:
                            form.type_trading === _TypeTrading.USD
                  ...
                  {!openMore ? (
                    <YStack>
                      ...
                      <InputPlusMinus
                        ...
                        pre={
                          form.type_trading === _TypeTrading.USD ? "USD" : "Lô"
                        }
                      />
                      {error?.volume && (
                        ...
                        <YStack gap={"$2"}>
                          ...
                          <YStack>
                            ...
                            <InputPlusMinus
                              ...
                              reference={1}
                              onChange={(value) =>
                                setForm({ percent_take_profit: value })
                              }
                              pre={"+%"}
                            />
                            {form.percent_take_profit &&
                              parseToNumber(form.percent_take_profit) > 0 && (
                          ...
                          <YStack>
                            ...
                            <InputPlusMinus
                              ...
                              reference={1}
                              onChange={(value) =>
                                setForm({ percent_stop_loss: value })
                              }
                              pre={"-%"}
                            />
                            {form.percent_stop_loss &&
                              parseToNumber(form.percent_stop_loss) > 0 && (
                                <Paragraph color={DefaultColor.slate[500]}>
                                  Gía cắt lỗ:
                                  {calculateProfit(
                                    props.price,
                ...
                <YStack gap={"$2"}>
                  <XStack gap="$2" alignItems="center" marginTop={"$4"}>
                    ...
                    <Button
                      ...
                      flex={1}
                      theme={
                        props.tradeType === _TradeType.BUY ? "blue" : "red"
                      }
                      backgroundColor={
                        props.tradeType === _TradeType.BUY
                          ? error.isError
                            ? DefaultColor.blue[300]
                            : DefaultColor.blue[500]
                          : error.isError
                          ? DefaultColor.red[300]
                    ...
                    >
                      <YStack alignItems="center" justifyContent="center">
                        <Paragraph theme="alt2" fontSize="$2" color="#fff">
                          Xác nhận{" "}
                          {props.tradeType === _TradeType.BUY ? "Mua" : "Bán"}
                          {form.type_trading === _TypeTrading.USD
                            ? ` ${form.volume} USD`
                            : ` ${form.volume} Lô`}
                        </Paragraph>
                  ...
                  >
                    ...
                    >
                      Phí: ~{hookInfoTrading.trans_fee.toFixed(2)} | Thực: ~
                      {hookInfoTrading.priceConvert.totalPrice.toFixed(2)} (Đòn
                      bẩy: {props.account?.lever?.min || 0}:
                      {props.account?.lever?.max || 0})
                    </Paragraph>
      ...
      <TransactionMoreInfo
        ...
        hookInfoTrading={hookInfoTrading}
      />
    </>
  );
};
