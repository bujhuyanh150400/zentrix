...
export const TYPE_CHART_SELECT = [
  {
    label: "Đường",
    unit: (
      <FontAwesome6 name="chart-line" size={sizeDefault.md} color="black" />
    ),
    value: _TypeChart.LINE,
  },
  {
    label: "Biểu đồ nến",
    unit: (
...
export default function TradingScreen() {
  ...
  const [openTransactionSheet, setOpenTransactionSheet] =
  ...
  //
  const { bid, ask, spread } = calculateBidAskSpread(
    priceRealtime?.price,
    queryItemSymbol.data ? queryItemSymbol.data.spread : ""
  );
  useEffect(() => {
                  ...
                  <YStack>
                    ...
                    {queryItemSymbol.data ? (
                      ...
                      >
                        ...
                        {queryItemSymbol.data.currency_quote
                          ? `vs ${queryItemSymbol.data.currency_quote}`
                          : ""}
                      </Paragraph>
                    ) : (
                      <SkeletonFade />
                ...
                <TouchableOpacity
                  onPress={() => {
                    if (account) {
                      ...
                      Alert.alert(
                        "Bạn cần mở tài khoản",
                        "Bạn cần mở tài khoản trước để có thể giao dịch được"
                      );
                    }
                  }}
                  style={[
                    styles.btn_trading,
                    { backgroundColor: DefaultColor.red[500] },
                  ]}
                >
                  <Paragraph fontSize={14} fontWeight={500} color={"white"}>
                    BÁN
                  </Paragraph>
                  <Paragraph fontSize={14} fontWeight={500} color={"white"}>
                    {bid}
