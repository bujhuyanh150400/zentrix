...
import {useGetAccountActive, useMutationRecharge, useRechargeAccountForm} from "@/services/account/hook";
import {_AccountType, RechargeAccountForm, RechargeAccountRequest} from "@/services/account/@types";
import {generateUniqueIdRecharge} from "@/libs/utils";
import {Keyboard, KeyboardAvoidingView, Platform, Text, TextInput, View, StyleSheet, Animated, TouchableWithoutFeedback, TouchableOpacity} from "react-native";
...
export default function RechargeScreen() {
    ...
    const {control, handleSubmit, setValue, formState: {errors}, getValues} = useRechargeAccountForm(account);
    ...
    const onSubmitStepOne = useCallback((data: RechargeAccountForm) => {
        if (account?.type === _AccountType.TEST_ACCOUNT) {
            mutate(data);
        } else {
    ...
    const onSubmitStepTwo = useCallback((data: RechargeAccountForm) => {
        const form = {
            ...data,
            ...option,
        };
    ...
    return (
        <>
                            ...
                            <Form
                                ...
                                <YStack gap="$3">
                                    ...
                                    <Paragraph fontSize={sizeDefault["sm"]} color={DefaultColor.slate[500]}>
                                        Min: {Number(account?.account_type.min || 0).toLocaleString("en-US")} USD
                                    </Paragraph>
                                    <Paragraph fontSize={sizeDefault["sm"]} color={DefaultColor.slate[500]}>
                                        Max: {Number(account?.account_type.max || 0).toLocaleString("en-US")} USD
                                    </Paragraph>
                                </YStack>
            ...
            {(step === _Step.STEP_2 && bankConfig.data && twPrice.money) ?
                <Animated.View style={{flex: 1, backgroundColor: DefaultColor.white, opacity: fadeAnim}}>
                    <YStack flex={1} gap="$2" padding={"$4"} alignItems="center" justifyContent={"space-between"}>
                        <YStack gap="$2" alignItems="center" >
                            <Text>
                                Quét mã để chuyển tiền vào tài khoản
                            </Text>
                            ...
                            <XStack alignItems={"center"} gap={"$2"} marginBottom={16}>
                                <Text>
                                    STK: {bankConfig.data.account_number}
                                </Text>
                                <TouchableOpacity style={styles.copy} onPress={async () => {
                                    if (bankConfig.data) {
                                        await Clipboard.setStringAsync(bankConfig.data.account_number);
                                        showMessage({
                            ...
                            <Image
                                ...
                                objectFit={"contain"}
                            />
                            <Text style={{fontSize: sizeDefault.lg, marginBottom: 20, fontWeight: 700}}>
                                {bankConfig.data.bank_name}
                            </Text>
                            <XStack alignItems={"center"} gap={"$2"} marginBottom={16}>
                                <Text>
                                    Nội dung: {option.transaction_code}
                                </Text>
                                <TouchableOpacity style={styles.copy} onPress={async () => {
                            ...
                            <Text>
                                Giá tiền cần nạp: {Number(getValues().money).toLocaleString("en-US")} USD
                            </Text>
                            <XStack alignItems={"center"} gap={"$2"}>
                                <Text>
                                    Giá tiền sau quy đổi: {option.amount_vnd.toLocaleString('en-US')} VND
                                </Text>
                                <TouchableOpacity style={styles.copy} onPress={async () => {
                                    await Clipboard.setStringAsync(option.amount_vnd.toFixed(0));
                                    showMessage({
                                        message: "Đã copy vào clipboard",
                                        type: 'info',
                                        duration: 3000,
                            ...
                            <Text>
                                Mệnh giá USD / VND: {Number(twPrice.money).toLocaleString('en-US')}
                            </Text>
                        </YStack>
                        <Button theme="yellow" fontWeight="bold" borderWidth={1} borderColor="$yellow10"
